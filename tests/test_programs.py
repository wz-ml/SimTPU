...
from source.programs.matmul import matmul_kernel, matmul_kernel_all_regs, setup as setup_gemm
...
def test_matmul_kernel_all_regs():
    torch.manual_seed(0)
    tpu = SimTPU(use_device=False)
    builder = ProgramBuilder()
    ...
    M, N, K = 256, 256, 256
    a_ptr, b_ptr, c_ptr, expected = setup_gemm(tpu, M, N, K)
    matmul_kernel_all_regs(builder, a_ptr, b_ptr, c_ptr, M, N, K)
